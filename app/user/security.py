...
def create_access_token(
    ...
    *,
    ...
    expire_at = now_utc() + timedelta(minutes=expires_minutes or settings.ACCESS_TOKEN_EXPIRE_MINUTES)
    payload = {
        'sub': str(user_id),
...
def create_refresh_token(
    ...
    *,
    ...
    if expires_minutes is not None:
        expire_at = now_utc() + timedelta(minutes=expires_minutes)
    else:
        expire_at = now_utc() + timedelta(days=expires_days or settings.REFRESH_TOKEN_EXPIRE_DAYS)
    jti = uuid4().hex
    payload = {
        'sub': str(user_id),
        'tv': int(token_version),
